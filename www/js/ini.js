...
// Учебные периоды
// Формат записи: ключ - дата последнего дня периода; далее указываются
// наименования итоговых отметок в том порядке, в котором они в таблице
// отметок и в табеле будут следовать за указанной в ключе датой (краткое
// наименование, полное наименование, дата первого дня периода, за который
// выставляется итоговая отметка). Даты первого и последнего дней периода
// можно указывать "с запасом", не обязательно точно.
const STPER = {
   "05.11": {
      ["1ч",  "Первая четверть",    "01.09"]      
   },
   "31.12": {
      ["2ч",  "Вторая четверть",    "06.11"],
      ["1п",  "Первое полугодие",   "01.09"]
   },
   "28.03": {
      ["3ч",  "Третья четверть",    "01.01"]      
   },
   "31.05": {
      ["4ч",  "Четвертая четверть", "29.03"],
      ["2п",  "Второе полугодие",   "01.01"],
      ["Год", "Учебный год",        "01.09"]
   }
};
