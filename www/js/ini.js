...
// Функция принимает аргумент - дату в формате d613 - и возвращает краткое
// наименование учебного периода типа "3ч" (первого встретившегося
// в массиве STER!), в котором находится эта дата. Если дата не принадлежит
// ни одному учебному периоду, возвращается undefined
const whereis = dt => {
   for (let k in DTSIT)
      if (dt >= DTSIT[k][2] && dt <= DTSIT[k][3]) return DTSIT[k][0];
   return undefined;
}
